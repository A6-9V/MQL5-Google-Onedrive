...
void OnTick()
{
  ...
  // We need the High/Low of the previous N bars (excluding the signal bar itself).
  int donStart = sigBar + 1;
  double dHighs[], dLows[];
  ArraySetAsSeries(dHighs, true);
  ArraySetAsSeries(dLows, true);

  if(CopyHigh(_Symbol, gSignalTf, donStart, gDonchianLookback, dHighs) != gDonchianLookback) return;
  if(CopyLow(_Symbol, gSignalTf, donStart, gDonchianLookback, dLows) != gDonchianLookback) return;

  double donHigh = dHighs[ArrayMaximum(dHighs)];
  double donLow  = dLows[ArrayMinimum(dLows)];
