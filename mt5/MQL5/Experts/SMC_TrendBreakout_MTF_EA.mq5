...
double CalculateLots(double slDistance, double balanceOrEquity, double freeMargin)
{
   ...
   //--- ⚡ Bolt: Apply margin clamping before normalization to avoid redundant calculations.
   //--- Use pre-calculated g_invMarginInitial to avoid expensive division.
   if(RiskClampToFreeMargin && g_invMarginInitial > 0)
   {
      double maxMarginLots = freeMargin * g_invMarginInitial;
      if(lots > maxMarginLots)
         lots = maxMarginLots;
   }

   //--- ⚡ Bolt: Consolidate rounding and clamping into a single execution path.
   //--- MathFloor with g_invLotStep handles the volume step normalization.
   //--- We add a small epsilon to lots * g_invLotStep to avoid precision issues with MathFloor.
   lots = MathFloor(lots * g_invLotStep + 0.00000001) * g_lotStep;
