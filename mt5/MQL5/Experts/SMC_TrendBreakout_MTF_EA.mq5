...
void OnTick()
{
   ...
   //--- ⚡ Bolt: Pass pre-fetched indicator data to trade functions to avoid redundant CopyBuffer() calls.
   //--- Execute trades
   if(buySignal) {
...
double CalculateTP(double price, double sl, bool isSell, double upperBand, double lowerBand)
{
   ...
   if(TPMode == TP_RR) {
      ...
      //--- ⚡ Bolt: Use pre-fetched indicator values instead of calling CopyBuffer again.
      //--- The check for valid data is now performed in OnTick() before this is called.
