...
void OnTick()
{
  ...
  // PERF: Lightweight check for a new bar on the signal timeframe.
  // This allows an early exit on most ticks, avoiding the expensive CopyRates call below.
  const int sigBar = (FireOnClose ? 1 : 0);
  datetime newBarTime = iTime(_Symbol, tf, sigBar);
  if(newBarTime == 0 || newBarTime == gLastSignalBarTime) // 0 if history is not ready
  {
    return;
  }
  // New bar detected, update the time and proceed.
  ...
  // Run once per signal bar -- check is now performed above, before CopyRates.
