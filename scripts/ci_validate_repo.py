...
def validate_and_collect_files() -> list[Path]:
    ...
    # Single pass to find and validate files using os.walk for efficiency
    for root, _, filenames in os.walk(str(MQL5_DIR)):
        for filename in filenames:
            ...
            # Check for NUL bytes using chunks for performance and memory efficiency
            try:
                with open(p, 'rb') as f:
                    found_nul = False
                    while True:
