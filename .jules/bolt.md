...
## 2026-02-04 - Optimized Lot Normalization and Margin Clamping
**Learning:** In MQL5 `CalculateLots` functions, applying margin-based clamping before the final normalization and volume limit checks (`MathFloor`, `MathMax`, `MathMin`) is more efficient. This ensures that the rounding and clamping logic is applied only once to the final lot value, avoiding redundant calculations if the initial lot size exceeds available margin. Additionally, replacing division by `SYMBOL_MARGIN_INITIAL` with multiplication by a cached inverse value further improves performance in the trade execution path.
**Action:** Consolidate lot size normalization into a single execution path by applying all constraints (risk-based, margin-based, and broker limits) before the final rounding step. Use cached inverse values for margin calculations.
