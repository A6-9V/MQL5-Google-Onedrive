...
## 2026-02-04 - Pre-calculating Trade Constants in MQL5
**Learning:** Significant performance gains in MQL5 backtesting are achieved by pre-calculating all derived trading constants in `OnInit()`. This includes inverse multipliers for lot steps and margin (to replace divisions with multiplications), combined clamping limits (merging symbol limits and user input limits), and fixed point offsets for SL/TP. These operations, while small, add up when executed millions of times in the tester.
**Action:** Always scan `CalculateLots`, `CalculateSL`, and `CalculateTP` for expressions involving constant inputs or symbol properties. Move these calculations to `OnInit()` and store the results in global variables for rapid access in the `OnTick` path.
