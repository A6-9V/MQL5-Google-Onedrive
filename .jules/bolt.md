...
## 2026-02-05 - Terminal State Caching and Time Filter Optimization
**Learning:** In high-frequency MQL5 EAs that execute every tick (e.g., for trailing stops), redundant calls to terminal environment APIs like `TerminalInfoInteger(TERMINAL_TRADE_ALLOWED)` and `MQLInfoInteger(MQL_TRADE_ALLOWED)` can consume significant CPU cycles. Caching these results for 1 second is safe and provides a measurable performance boost. Additionally, optimizing time-window checks by comparing seconds-since-midnight against pre-calculated constants in `OnInit` is faster than repeated hour-extraction math.
**Action:** Always implement 1-second state caching for non-critical terminal environment checks in `OnTick` hot paths and pre-calculate time filter bounds as seconds-since-midnight during initialization.
