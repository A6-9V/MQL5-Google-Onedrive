...
## 2026-02-14 - ProcessPoolExecutor Overhead for Lightweight Tasks
**Learning:** Using `ProcessPoolExecutor` for parallelizing a small number of very short-lived tasks (e.g., executing a handful of subprocess commands) is often slower than sequential execution. The overhead of process creation, inter-process communication (IPC), and result serialization can easily exceed the actual execution time of the tasks themselves. In this codebase, switching `scripts/test_automation.py` to sequential execution reduced runtime from ~4.2s to ~2.9s (~30% reduction).
**Action:** Always evaluate the granularity of tasks before using multiprocessing. If tasks are lightweight and few in number, prioritize sequential execution to avoid process management overhead.
