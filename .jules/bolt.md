...
## 2026-02-04 - Caching Dynamic Risk Limits and Redundant Time Calls
**Learning:** Significant performance gains in high-frequency trading bots (EAs) can be achieved by caching dynamically calculated risk limits (like daily loss/profit targets based on account balance) instead of recalculating them on every tick. Additionally, fetching `TimeCurrent()` once at the start of `OnTick` and passing it to sub-functions (like `IsTradingAllowed` or `UpdateDailyStatistics`) avoids multiple expensive system calls per tick. Finally, ensuring custom logging functions match their single-parameter signatures avoids potential compilation errors that might be hidden by `Print()`'s variadic nature.
**Action:** Always cache risk-related currency limits in global variables and update them only when the balance changes (e.g., in `OnTrade`). Implement a "time-passing" pattern to reuse a single `TimeCurrent()` value across the tick's execution path.
