...
## 2026-02-05 - Markdown Rendering Cache in Flask Dashboards
**Learning:** In simple Flask-based monitoring dashboards, reading and rendering markdown files (like README.md) on every request can become a major latency driver (~75% of total request time). Implementing a simple cache that tracks the file's modification time (`mtime`) allows for near-instant responses while still supporting dynamic content updates without a server restart.
**Action:** Always wrap disk-based template or content reads in a lazy-loading cache with modification tracking when building performance-sensitive internal tools.
