...
## 2026-01-20 - Robust Event-Driven Daily Statistics in MQL5
**Learning:** Moving daily statistics (profit, loss, trade count) from the per-tick OnTick hot path to event handlers like OnTrade and OnTimer significantly improves performance by reducing redundant HistorySelect and deal-looping operations. To ensure correctness, these statistics must be initialized in OnInit, and a robust day-rollover reset mechanism must be maintained in the gatekeeper checks (e.g., CheckDailyLimits) to clear stats as soon as a new calendar day is detected.
**Action:** Relocate heavy history scanning to OnTrade/OnTimer/OnInit and consolidate all daily stat updates into a single optimized function. Always use EventSetTimer in OnInit and EventKillTimer in OnDeinit when using the OnTimer handler.
