...
## 2026-02-05 - Caching Account Statistics and API Consolidation
**Learning:** Frequent calls to AccountInfoDouble() and TimeCurrent() in the OnTick hot path introduce significant overhead, especially during high volatility. Caching balance-dependent limits (like daily profit/loss in currency) and consolidating time retrieval to a single call at the start of OnTick (passing it down as a parameter) significantly reduces the number of terminal API calls per tick.
**Action:** Always fetch TimeCurrent() once per OnTick and pass it to sub-functions. Cache any limits or values that depend on account state inside event handlers that only fire when that state changes (e.g., OnTrade, OnTimer) rather than recalculating them on every price tick.
